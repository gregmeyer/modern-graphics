...
- **Open hero + `freeform_canvas`:** Pretext runs only where `pretext_slot` is emitted (headline, subhead, insight callout, etc.). Wireframe or diagram SVG passed as `freeform_canvas` is separate markup; Pretext does not process it unless you add your own `.pretext-slot` nodes inside that snippet. Typography and diagram are siblings in the hero body, not nested SVG text. The insight quote callout now uses refined slot and stacking behavior so italic quote text does not overlap the quote icon lane.
- **Story slide mini-tile:** Headline and subline can use Pretext inside SVG `<foreignObject>`. In Pretext mode, the mini-tile reserves refined `foreignObject` heights to prevent clipping for multi-line text; if you change copy or font sizes drastically, re-check layout in the mini-tile generator (`modern_graphics/diagrams/story_slide.py`, `_build_static_mini_tile`).
