...
def main():
    ...
    # Chart layouts (line/bar/grouped-bar/horizontal-bar/stacked-bar/grouped-stacked-bar/stacked-area/pie/donut/sankey/cohort)
    create_layout.add_argument('--labels', help='Chart x-axis / category labels (comma-separated)')
    create_layout.add_argument('--series-json', help='Chart multi-series data: JSON array (e.g. \'[{"name":"A","values":[1,2,3]}]\'). For grouped-stacked add "stack": "<group>". Accepts @path/to/file.json.')
    create_layout.add_argument('--links-json', help='Sankey links: JSON array (e.g. \'[{"from":"A","to":"B","value":10}]\'). Accepts @path/to/file.json.')
    create_layout.add_argument('--nodes', help='Sankey nodes (comma-separated, optional — derived from links otherwise)')
    create_layout.add_argument('--cohorts-json', help='Cohort rows: JSON array (e.g. \'[{"date":"Jan 1","size":100,"values":[95.0,30.0]}]\'). Accepts @path/to/file.json.')
    create_layout.add_argument('--period-labels', help='Cohort period column headers (comma-separated)')
    create_layout.add_argument('--subtitle', help='Chart subtitle (below title)')
    create_layout.add_argument('--x-label', help='Chart x-axis label')
    create_layout.add_argument('--y-label', help='Chart y-axis label')
    create_layout.add_argument('--no-legend', action='store_true', help='Hide chart legend (default: shown for multi-series charts)')
